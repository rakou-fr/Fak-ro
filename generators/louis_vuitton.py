...
TEMPLATE = "templates/lv_1.png"
OUTPUT   = "final/lv"


async def generate(fields, is_free):
    num_art = fields["num_art"]
    nom_art = fields["nom_art"]
    qte = fields["qte"]
    prix = fields["prix"]
    ...
    f1 = ImageFont.truetype("calibri-font-family/calibri-regular.ttf", 55)
    f2 = ImageFont.truetype("calibri-font-family/calibri-bold.ttf", 55)

    # Layer 1: field
    _txt0 = num_art
    t(draw, (W // 2 + int(-1500 * 1), int(1637 * 1)), _txt0, f1, (8, 9, 11), 4)

    # Layer 2: field
    _txt1 = wrap(nom_art, 26)
    t(draw, (W // 2 + int(-1200 * 1), int(1637 * 1)), _txt1, f1, (8, 9, 11), 4)

    # Layer 3: field
    _txt2 = qte
    t(draw, (W // 2 + int(-200 * 1), int(1637 * 1)), _txt2, f1, (8, 9, 11), 4)

    # Layer 4: formula
    _txt3 = f"{prix},00"
    t(draw, (W // 2 + int(500 * 1), int(1637 * 1)), _txt3, f1, (8, 9, 11), 4)

    # Layer 5: formula
    _txt4 = f"{prix},00"
    t(draw, (W // 2 + int(1270 * 1), int(1637 * 1)), _txt4, f1, (8, 9, 11), 4)

    # Layer 6: formula
    _txt5 = f"{prix},00      EUR\n\n{prix},00      EUR\n\n\n            0,00  EUR"
    t(draw, (W // 2 + int(1180 * 1), int(1880 * 1)), _txt5, f1, (8, 9, 11), 4)

    # Layer 7: random_text
    _txt6 = f"0{random.randint(10000, 99999)}/0{random.randint(10000, 99999)}                                                       {random.randint(10, 28)}/{random.randint(1, 9)}/20{random.randint(20, 23)} - {random.randint(10, 23)}:{random.randint(10, 59)}"
    t(draw, (W // 2 + int(-580 * 1), int(1410 * 1)), _txt6, f2, (8, 9, 11), 4)
