...
TEMPLATE = "templates/FactureAmi.png"
OUTPUT   = "final/ami"
...
async def generate(fields, is_free):
    ...
    desc = fields["desc"]
    qte = fields["qte"]
    code_postale = fields["code_postale"]
    prix = fields["prix"]
    ...
    f1 = ImageFont.truetype("calibri-font-family/Caladea-Regular.ttf", 29)

    # Layer 1: field
    _txt0 = date
    t(draw, (W // 2 + int(-201 * 1), int(881 * 1)), _txt0, f1, (9, 9, 9), 4)

    # Layer 2: field
    _txt1 = code_postale
    t(draw, (W // 2 + int(35 * 1), int(946 * 1)), _txt1, f1, (9, 9, 9), 4)

    # Layer 3: random_text
    _txt2 = f"{random.randint(10, 99)} {random.randint(100, 999)}"
    t(draw, (W // 2 + int(-35 * 1), int(980 * 1)), _txt2, f1, (9, 9, 9), 4)

    # Layer 4: field
    _txt3 = wrap(desc, 27)
    t(draw, (W // 2 + int(-260 * 1), int(1135 * 1)), _txt3, f1, (9, 9, 9), 4)

    # Layer 5: field
    _txt4 = qte
    t(draw, (W // 2 + int(118 * 1), int(1135 * 1)), _txt4, f1, (9, 9, 9), 4)

    # Layer 6: formula
    ht = float(prix)*0.8333333333
    _txt5 = f"{ht:.2f} \u20ac"
    t(draw, (W // 2 + int(325 * 1), int(1135 * 1)), _txt5, f1, (9, 9, 9), 4)

    # Layer 7: formula
    ht = float(prix)*0.8333333333
    _txt6 = f"{ht:.2f} \u20ac"
    t(draw, (W // 2 + int(510 * 1), int(1135 * 1)), _txt6, f1, (9, 9, 9), 4)

    # Layer 8: formula
    ht = float(prix)*0.8333333333
    _txt7 = f"{ht:.2f} \u20ac"
    t(draw, (W // 2 + int(510 * 1), int(1250 * 1)), _txt7, f1, (9, 9, 9), 4)

    # Layer 9: formula
    tva = float(prix)*0.1666666666
    _txt8 = f"{tva:.2f} \u20ac"
    t(draw, (W // 2 + int(510 * 1), int(1330 * 1)), _txt8, f1, (9, 9, 9), 4)

    # Layer 10: formula
    _txt9 = f"{prix:.2f} \u20ac"
    t(draw, (W // 2 + int(510 * 1), int(1412 * 1)), _txt9, f1, (9, 9, 9), 4)
