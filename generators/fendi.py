...
TEMPLATE = "templates/fendi.png"
OUTPUT   = "final/fendi"


async def generate(fields, is_free):
    article_num = fields["article_num"]
    ...
    f1 = ImageFont.truetype("calibri-font-family/liberationserif-regular.ttf", 25)
    f2 = ImageFont.truetype("calibri-font-family/liberationserif-regular.ttf", 23)
    f3 = ImageFont.truetype("calibri-font-family/liberationserif-regular.ttf", 18)

    # Layer 1: random_address
    _txt0 = adresse()
    t(draw, (W // 2 + int(-675 * 1), int(325 * 1)), _txt0, f1, (9, 9, 9), 4)

    # Layer 2: random_text
    _txt1 = f"{random.randint(10, 28)}/0{random.randint(1, 10)}/{random.randint(2021, 2023)} {random.randint(10, 17)}:{random.randint(10, 59)}:{random.randint(10, 59)}\n\n{random.randint(10, 20)}{random.choice(LETTRES)}{random.choice(LETTRES)}{random.choice(LETTRES)}{random.choice(LETTRES)}{random.choice(LETTRES)}{random.choice(LETTRES)}\n{random.randint(10, 20)}{random.choice(LETTRES)}{random.choice(LETTRES)}{random.choice(LETTRES)}{random.choice(LETTRES)}{random.choice(LETTRES)}{random.choice(LETTRES)}"
    t(draw, (W // 2 + int(-475 * 1), int(200 * 1)), _txt1, f2, (9, 9, 9), 4)

    # Layer 3: formula
    _txt2 = f"{article_num}         {designation}         {description}"
    t(draw, (W // 2 + int(-675 * 1), int(635 * 1)), _txt2, f3, (9, 9, 9), 4)

    # Layer 4: formula
    _txt3 = f"{taille}                               {prix:.2f}                                              {prix:.2f}"
    t(draw, (W // 2 + int(130 * 1), int(635 * 1)), _txt3, f3, (9, 9, 9), 4)

    # Layer 5: formula
    tva = float(prix)*0.166666
    _txt4 = f"{prix:.2f}\n\n\n{prix:.2f}\n{tva:.2f}\n{prix}"
    t(draw, (W // 2 + int(592 * 1), int(730 * 1)), _txt4, f3, (9, 9, 9), 4)
